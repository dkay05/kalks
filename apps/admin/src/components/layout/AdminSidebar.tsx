...
const NAV_ITEMS: NavItem[] = [
  ...
  { label: 'Account types', href: '/account-types', icon: Layers, perm: 'config.view' },
  {
    ...
    children: [
      ...
      { label: 'Referral Bonus Campaigns', href: '/config/referral-bonus-campaign', perm: 'referral_campaigns.view' },
    ],
  },
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center h-14 px-3 border-b border-border-primary/40">
        {/* White mark on the dark theme, black mark on the light theme;
            globals.css toggles the pair by data-theme. */}
        {collapsed ? (
          <>
            <img src={BRAND_LOGO_REVERSED} alt={BRAND_NAME} className="brand-logo-dark w-full h-6 object-contain" />
            <img src={BRAND_LOGO_INK} alt="" aria-hidden className="brand-logo-light w-full h-6 object-contain" />
          </>
        ) : (
          <Link href="/" className="flex items-center min-w-0">
            <img src={BRAND_LOGO_REVERSED} alt={BRAND_NAME} className="brand-logo-dark h-8 w-auto object-contain shrink-0" />
            <img src={BRAND_LOGO_INK} alt="" aria-hidden className="brand-logo-light h-8 w-auto object-contain shrink-0" />
          </Link>
        )}
        <button
