 ...
 *
 * Priority: TRADER_API_PROXY_TARGET | GATEWAY_URL | GATEWAY_INTERNAL_URL |
 * INTERNAL_API_URL (strip /api/v1) | NEXT_PUBLIC_GATEWAY_ORIGIN | localhost.
 * Docker Compose sets INTERNAL_API_URL=http://gateway:8000/api/v1 — that must be honored here.
 * GATEWAY_INTERNAL_URL is the name .env.local and src/config/env.ts already use,
 * so it is accepted here too; without it a deployment that sets only that name
 * would silently fall through to the localhost default and every call would fail.
 */
